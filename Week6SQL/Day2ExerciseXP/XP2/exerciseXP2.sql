...
-- 6)Write a query to get the address, and the phone number of all customers living in the Texas district, these details can be found in the “address” tabl
...
SELECT film_id,title, description, length, rental_rate FROM film WHERE title = 'Marley and me'
-- is not in the BD.
...
-- SELECT * FROM film
-- WHERE rental_rate = 
-- (SELECT MIN(rental_rate) FROM film) LIMIT 10

--11) Not satisfied with the results. Write a query which will find the next 10 cheapest movies.
SELECT * FROM film ORDER BY rental_rate LIMIT 20 OFFSET 10
...
SELECT first_name, last_name, amount, payment_date FROM customer INNER JOIN payment ON customer.customer_id = payment.customer_id
ORDER BY customer.customer_id ASC

-- 13) You need to check your inventory. Write a query to get all the movies which are not in inventory.
SELECT f.film_id, f.title FROM film AS f LEFT JOIN inventory AS i ON f.film_id = i.film_id 
...
SELECT c.customer_id, c.first_name, c.last_name, p.amount, p.payment_date, s.staff_id, s.first_name, s.last_name 
FROM customer AS c 
